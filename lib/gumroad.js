...
function validateWebhookSignature(configuredSecret, providedSecret) {
    ...
    if (configBuf.length !== providedBuf.length) return false;

    // Use timing-safe comparison to prevent timing attacks
    try {
        return crypto.timingSafeEqual(configBuf, providedBuf);
    } catch (err) {
        return false;
    }
}
